		...
		if event.pressed and get_node("Grass")==null:
			...
			if distance<=50 and !tilled and planted==false:
				...
				if result:
					...
					if Global.player_direction==Vector2(1,0): 
						if get_node(soil_path+str(number-1)) ==null:
							return
					...
					if Global.player_direction==Vector2(0,1):
						if get_node(soil_path+str(number-row_soil_count)) ==null:
							return
						if get_node(soil_path+str(number-row_soil_count)).tilled==true:
							...
							if get_node(soil_path+str(number-row_soil_count)).adjusted!=true :
								get_node(soil_path+str(number-row_soil_count)).get_node("AnimatedSprite2D").play("rect_tilled")
								#print("RECT")
								if get_node(soil_path+str(number-row_soil_count)).planted==true:
									get_node(soil_path+str(number-row_soil_count)).get_node("AnimatedSprite2D").play("seeds")
									#print("SEEDS")
								if get_node(soil_path+str(number-row_soil_count)).watered==true:
									get_node(soil_path+str(number-row_soil_count)).get_node("AnimatedSprite2D").play("watered_soil")
								
								if get_node(soil_path+str(number-row_soil_count)).watered==true and get_node(soil_path+str(number-23)).planted==true:
									get_node(soil_path+str(number-row_soil_count)).get_node("AnimatedSprite2D").play("watered_seeds")
									#print("watered_seeds")
					...
					if Global.player_direction==Vector2(0,-1) and get_node(soil_path+str(number+row_soil_count))!=null:
						...
						if get_node(soil_path+str(number+row_soil_count)).tilled==true:
							...
							if get_node(soil_path+str(number+row_soil_count)).adjusted!=true :
								...
								if get_node(soil_path+str(number+row_soil_count)).watered==true:
									get_node(soil_path+str(number+row_soil_count)).get_node("AnimatedSprite2D").play("watered_soil")
									get_node(soil_path+str(number+row_soil_count)).get_node("AnimatedSprite2D").scale.y=0.15
									#print("watered_seeds")
								if get_node(soil_path+str(number+row_soil_count)).watered==true and get_node(soil_path+str(number+23)).planted==true:
									get_node(soil_path+str(number+row_soil_count)).get_node("AnimatedSprite2D").play("watered_seeds")
								get_node(soil_path+str(number+row_soil_count)).adjusted=true
